...
# Optuna best trial hyperparameters, ty Neil
BEST = {
    "depth": 4,
    "base_width": 512,
    "latent_dim": 64,
...
def train_and_score_autoencoder(
    ...
    *,
    ...
    # Where model artifacts will be saved
    model_output_dir = get_dataset_dir(model_dir, dataset)
    os.makedirs(model_output_dir, exist_ok=True)
    compute_device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # Use GPU if available
    ...
    X_benign_train = X_train_split[mask_benign] # X_fit
    ...
    # Optimizer + loss (Huber is robust to occasional outliers)
